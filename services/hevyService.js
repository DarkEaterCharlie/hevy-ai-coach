  ...
  try {
    ...
    while (keepFetching) {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Hevy API returned HTTP ${response.status}: ${errorText}`,
        );
      }
    ...
    console.log(
      `   [Hevy] Downloaded ${allRoutines.length} routines total. Filtering by folder...`,
    );

    // Filter by target folder
    const folderRoutines = allRoutines.filter(
      (r) => String(r.folder_id) === String(folderId),
    );
...
async function getLastWorkouts(apiKey, count = 5) {
  try {
    const response = await fetch(
      `https://api.hevyapp.com/v1/workouts?page=1&pageSize=${count}`,
      {
        headers: { "api-key": apiKey },
      },
    );
    ...
    if (!response.ok)
      throw new Error(
        `Hevy API workout history request failed: ${response.status}`,
      );
    ...
    return workouts.map((w) => ({
      ...
      exercises: w.exercises.map((ex) => ({
        name: ex.title,
        sets: ex.sets.map(
          (s) => `${s.weight_kg}kg x ${s.reps} (RPE ${s.rpe || "?"})`,
        ),
      })),
    }));
  } catch (error) {
