...
async function sync1RMToSheets() {
    const apiKey = process.env.HEVY_API_KEY;
    const userBw = parseFloat(process.env.USER_BW);
    const spreadsheetId = process.env.SPREADSHEET_ID;

    if (!apiKey || !userBw || !spreadsheetId) {
        console.error("❌ CHYBA: Zkontroluj .env! Chybí API klíč, USER_BW nebo SPREADSHEET_ID.");
        return;
    }
        ...
        allWorkouts.forEach(workout => {
            ...
            workout.exercises.forEach(exercise => {
                ...
                exercise.sets.forEach(set => {
                    ...
                    const current1RM = zvednutaVaha * (1 + efektivniOpakovani / 30);
                    ...
                    if (!best1RM[exId] || current1RM > best1RM[exId].rm) {
                        best1RM[exId] = {
                            ...
                            date: new Date(workout.start_time).toLocaleDateString('cs-CZ'),
                            info: `${set.weight_kg || 0}kg x ${set.reps} @ RPE ${rpe}`
                        };
                    }
                });
            });
